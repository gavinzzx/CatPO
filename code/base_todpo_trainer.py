...
class BaseDPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.lambda_kl = getattr(args, "lambda_kl", 0.0)
    ...
    def dpo_loss(
        ...
        beta_eff: Optional[torch.FloatTensor] = None, 
        token_kl: Optional[torch.FloatTensor] = None    
        ...
        if token_kl is not None and self.lambda_kl > 0:     
            losses = losses + self.lambda_kl * token_kl
    ...
    def _get_batch_logps(
        ...
        token_weights: Optional[torch.FloatTensor] = None  # ToDPO new version
    ) -> torch.FloatTensor:
        ...
        if not self.is_encoder_decoder:
            labels = labels[:, 1:].clone()  #labels[:, 1:]to remove the 2rd dimension（sequence）of the first token
            logits = logits[:, :-1, :]  


            '''
            print(f"Logits 【values】---after slicing:, {logits}")
            '''
        loss_mask = labels != self.label_pad_token_id

        labels[labels == self.label_pad_token_id] = 0

        per_token_logps = torch.gather(logits.log_softmax(-1), dim=2, index=labels.unsqueeze(2)).squeeze(2)        # => shape


        if token_weights is not None:
            token_weights = token_weights[:, 1:]            # shift， labels shape
            # print("token_weights.shape:",token_weights.shape)
            # print(f"token_weights【values】---after slicing:, {token_weights}")            
            # print("per_token_logps.shape:",per_token_logps.shape)
            # print(f"per_token_logps【values】---after slicing:, {per_token_logps}")            
            if token_weights.shape != per_token_logps.shape:
                raise ValueError("token_weights shape does not match per_token_logps shape after shifting.")
            per_token_logps = per_token_logps * token_weights
    ...
    # =========  New Version of  evaluation_loop  =========
    def evaluation_loop(
        self,
        ...
        if self._stored_metrics["eval"]:
            for key, values in self._stored_metrics["eval"].items():
                output.metrics[key] = torch.tensor(values, dtype=torch.float32).mean().item()
    ...
    def _get_weighted_kl(self, policy_logits, ref_logits, labels, token_weights):
        labels_ = labels[:, 1:]
        mask     = labels_ != self.label_pad_token_id
        policy_logp = F.log_softmax(policy_logits[:, :-1, :], -1)
        ref_logp    = F.log_softmax(ref_logits[:, :-1, :], -1)
        kl = (policy_logp.exp() * (policy_logp - ref_logp)).sum(-1)   # [B, T]
        w  = token_weights[:, 1:]
        return ((kl * w) * mask).sum(-1)          # [B]